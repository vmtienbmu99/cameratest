  ...
  window.apiPost = function(action, data){
    return new Promise(function(resolve){
      ...
      const timer = setTimeout(function(){
        if (finished) return;
        cleanup();
        resolve({ok:false, timeout:true, requestId: requestId, message:'Hết thời gian chờ phản hồi từ Apps Script. RequestID: ' + requestId + '. Hãy kiểm tra sheet BACKEND_LOG trong file tổng.'});
      }, 180000);
